...
export const getCourses = async (req, res) => {
    try {
        const curriculum = await Curriculum.findById(req.params.savedCurriculum_id)
...
export const addCourse = async (req, res) => {
    try {
        ...
        const idNewCourse = curriculum.coures[curriculum.courses.length-1]._id
...
export const updateCourse = async (req, res) => {
    try {
        ...
        curriculum.coures.id(id).code = code
        curriculum.coures.id(id).description = description
        curriculum.coures.id(id).semester = semester
        curriculum.coures.id(id).year = year
        curriculum.coures.id(id).lectureHours = lectureHours
        curriculum.coures.id(id).labHours = labHours
